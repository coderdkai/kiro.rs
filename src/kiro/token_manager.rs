...
impl MultiTokenManager {
    ...
    /// 获取指定凭据的调用上下文（Admin 验活使用）。
    ///
    /// 不参与负载均衡，不切换到其他凭据；如果指定凭据不可用或刷新失败，直接返回错误。
    ...
    pub fn report_failure(&self, id: u64) -> bool {
        let (result, was_disabled) = {
            ...
            (entries.iter().any(|e| !e.disabled), newly_disabled)
        };
        self.save_stats_debounced();
        if was_disabled {
            if let Err(e) = self.persist_credentials() {
                tracing::warn!("凭据禁用后持久化失败: {}", e);
            }
        }
        result
    }
    ...
    pub fn delete_credential(&self, id: u64) -> anyhow::Result<()> {
        ...
        // 从数据库删除
        {
            ...
            if tokio::runtime::Handle::try_current().is_ok() {
                tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(async { database::credentials::delete(&pool, id).await })
                })?;
            } else {
                let runtime = tokio::runtime::Runtime::new()?;
...
mod tests {
    ...
    async fn test_add_credential_reject_duplicate_refresh_token() {
        ...
        let manager =
            MultiTokenManager::new(config, vec![existing], None, test_db_pool().await).unwrap();
    ...
    async fn test_add_credential_api_key_and_oauth_coexist() {
        ...
        let manager =
            MultiTokenManager::new(config, vec![oauth_cred], None, test_db_pool().await).unwrap();
    ...
    async fn test_multi_token_manager_api_key_with_kiro_api_key_not_disabled() {
        ...
        let manager =
            MultiTokenManager::new(config, vec![cred], None, test_db_pool().await).unwrap();
        assert_eq!(manager.total_count(), 1);
        assert_eq!(manager.available_count(), 1);
    }
