#!/usr/bin/env python3
"""
Kiro / AWS Builder ID 自动注册 (独立版本)
纯协议实现 + IMAP 邮箱验证码获取

依赖安装:
    pip install curl_cffi cbor2 jwcrypto python-dotenv
"""
...
class KiroRegister:
    def __init__(self,proxy=None,tag="REG"):
        ...
        self.s=curl_requests.Session(impersonate="chrome131", timeout=120)
    ...
    def step2_get_wsh(self,redir_url):
        self.log("Step 2: 重定向链 + portal.sso...")

        # ── 2a: 从 Step 1 redirect URL 中提取 OIDC 参数 ──
        p1 = urlparse(redir_url)
        qs1 = parse_qs(p1.query)
        oidc_cb = qs1.get("callback_url", [None])[0]
        if not oidc_cb:
            self.log("  ❌ redirect URL 中无 callback_url"); return False

        # 提取 OIDC authorize 所需参数
        _request_uri = qs1.get("request_uri", [None])[0]
        _client_id = qs1.get("client_id", [None])[0]
        _oidc_state = qs1.get("state", [None])[0]
        _redirect_uri = qs1.get("redirect_uri", [None])[0]
        _response_type = qs1.get("response_type", [None])[0] or "code"

        # ★ 新增: 调 oidc/authorize 获取 orchestrator_id
        oid = None
        cb = oidc_cb
        if _request_uri and _client_id:
            self.log("  2a: 调 oidc/authorize 获取 orchestrator_id...")
            oidc_auth_url = (
                f"https://oidc.us-east-1.amazonaws.com/authorize?"
                f"request_uri={url_quote(_request_uri)}"
                f"&client_id={url_quote(_client_id)}"
            )
            if _oidc_state:
                oidc_auth_url += f"&state={url_quote(_oidc_state)}"
            if _response_type:
                oidc_auth_url += f"&response_type={_response_type}"
            if _redirect_uri:
                oidc_auth_url += f"&redirect_uri={url_quote(_redirect_uri)}"
            try:
                r_oidc = self.s.get(oidc_auth_url, headers=UA, allow_redirects=False)
                if r_oidc.status_code in (301, 302, 303, 307):
                    loc = r_oidc.headers.get("location", "")
                    p_loc = urlparse(loc)
                    qs_loc = parse_qs(p_loc.query)
                    oid = qs_loc.get("orchestrator_id", [None])[0]
                    if oid:
                        self.log(f"  ✅ orchestrator_id={oid[:60]}...")
                        # 同时更新 callback_url (OIDC 可能返回更完整的)
                    else:
                        self.log(f"  ⚠️ oidc/authorize 302 但无 orchestrator_id: {loc[:200]}")
                else:
                    self.log(f"  ⚠️ oidc/authorize status={r_oidc.status_code}")
            except Exception as e:
                self.log(f"  ⚠️ oidc/authorize 异常: {e}")

        if not oid:
            self.log("  ⚠️ 未获取到 orchestrator_id, Step 12 token 交换可能失败")

        # ── 2b: 跟随重定向链获取 cookies ──
        self.log("  2b: 跟随重定向链获取 cookies...")
        r = self.s.get(redir_url, headers=UA, allow_redirects=True)
        self._capture_cookies(r)
        final_url = str(r.url)
        self.log(f"  最终 URL: {final_url[:120]}")

        # ── 2c: 调用 portal.sso 获取 csrfToken + 与之配对的 wsh ──
        if oid:
            vr = (f"https://view.awsapps.com/start/#/"
                  f"?callback_url={url_quote(cb)}&orchestrator_id={oid}")
        else:
            vr = (f"https://view.awsapps.com/start/#/"
                  f"?callback_url={url_quote(cb)}")
        pu = (f"https://portal.sso.us-east-1.amazonaws.com/login"
              f"?directory_id=view&redirect_url={url_quote(vr)}")

        self.log("  2c: portal.sso/login...")
        ...
        try:
            ...
            # 从 portal.sso redirect 提取 wsh
            m = re.search(r"workflowStateHandle=([^&#]+)", redir)
            if m:
                self.wsh = m.group(1)
                self._login_wsh = self.wsh
                self.log(f"  ★ portal.sso wsh={self.wsh} (与 csrfToken 配对)")
                # 跟随 portal.sso 的 redirectUrl 加载 signin.aws login 页面
                ...
                self.log(f"  ⚠️ portal.sso redirect 无 wsh: {redir[:200]}")
        except Exception as e:
            self.log(f"  ❌ portal.sso: {e}")
        ...
        if not self.wsh:
            self.log("  ❌ 未获取到 wsh"); return False
        return True
    ...
    def step12_get_tokens(self):
        ...
        self.log(f"  [DEBUG] 初始 orchestrator_id={orch_id[:60] if orch_id else 'None'}...")
        if sso_redirect:
            ...
            if not new_orch:
                # 尝试从 callback_url 内部提取
                cb_from_frag = fqs.get("callback_url", [None])[0]
                if cb_from_frag:
                    p_cb2 = urlparse(cb_from_frag)
                    qs_cb2 = parse_qs(p_cb2.query)
                    new_orch = qs_cb2.get("orchestrator_id", [None])[0]
            if new_orch:
                orch_id = new_orch
                self.log(f"  ★ 更新 orchestrator_id={orch_id[:60]}...")

        if not orch_id:
            self.log(f"  ❌ orchestrator_id 为空，无法继续")
            return None
...
# ═══════════════════════════════════════════
#  iCloud HME (Hide My Email) + IMAP 验证码
# ═══════════════════════════════════════════
import imaplib
...
ICLOUD_DSID = os.getenv("ICLOUD_DSID", "")
ICLOUD_PARTITION = os.getenv("ICLOUD_PARTITION", "")
ICLOUD_COOKIES = os.getenv("ICLOUD_COOKIES", "")
HME_LABEL = os.getenv("HME_LABEL", "kiro")

def generate_hme(label=None):
    """调用 iCloud API 创建一个新的 Hide My Email 地址。
    返回 xxx@privaterelay.appleid.com 格式的邮箱地址。
    邮件会自动转发到 IMAP_EMAIL 配置的真实邮箱。"""
    dsid = ICLOUD_DSID
    partition = ICLOUD_PARTITION
    cookies = ICLOUD_COOKIES
    label = label or HME_LABEL

    if not dsid or not partition or not cookies:
        missing = []
        if not dsid: missing.append("ICLOUD_DSID")
        if not partition: missing.append("ICLOUD_PARTITION")
        if not cookies: missing.append("ICLOUD_COOKIES")
        raise RuntimeError(f"缺少 iCloud HME 配置: {', '.join(missing)}. 请在 .env 中配置")

    base_url = f"https://p{partition}-maildomainws.icloud.com"
    ...
    r = curl_requests.post(
        f"{base_url}/v1/hme/generate?{params}",
        headers=headers,
        data=json.dumps({"langCode": "en-us"}),
        impersonate="chrome131",
    )
    gen = r.json()
    if not gen.get("success"):
        raise RuntimeError(f"HME generate 失败: {json.dumps(gen, ensure_ascii=False)}")

    hme_email = gen["result"]["hme"]
    print(f"[HME] 已生成: {hme_email}")

    # Step 2: Reserve
    print(f"[HME] 激活 (label={label})...")
    r2 = curl_requests.post(
        f"{base_url}/v1/hme/reserve?{params}",
        headers=headers,
        data=json.dumps({"hme": hme_email, "label": label, "note": ""}),
        impersonate="chrome131",
    )
    res = r2.json()
    if not res.get("success"):
        raise RuntimeError(f"HME reserve 失败: {json.dumps(res, ensure_ascii=False)}")

    print(f"[HME] ✅ 激活成功: {hme_email}")
    return hme_email

def _decode_mime(s):
    if not s:
        return ""
    parts = mime_decode_header(s)
    result = []
...
def wait_for_otp(email_address=None, timeout=120, tag=""):
    """通过 IMAP 轮询收件箱，提取 AWS 验证码。

    email_address: 注册使用的邮箱 (用于过滤)
    """
    if not IMAP_EMAIL or not IMAP_PASSWORD:
        prefix = f"[{tag}] " if tag else ""
        print(f"{prefix}❌ .env 中未配置 IMAP_EMAIL / IMAP_PASSWORD")
        return None
    ...
    # 记录开始时的最新 UID，之后只检查更新的邮件
    try:
        mail = imaplib.IMAP4_SSL(IMAP_HOST, IMAP_PORT)
        mail.login(IMAP_EMAIL, IMAP_PASSWORD)
        mail.select("INBOX")
        status, msg = mail.uid("search", None, "ALL")
        initial_uids = set(msg[0].split()) if status == "OK" and msg[0] else set()
        mail.logout()
    except Exception as e:
        print(f"{prefix}⚠️ 获取初始 UID 失败: {e}")
        initial_uids = set()
    ...
    while time.time() - start < timeout:
        try:
            ...
            status, msg = mail.uid("search", None, "ALL")
            if status != "OK" or not msg[0]:
                mail.logout()
                time.sleep(3)
                continue

            current_uids = set(msg[0].split())
            # 只检查新邮件 (不在 initial_uids 中的)
            new_uids = current_uids - initial_uids - seen_uids
            ...
            for uid in sorted(new_uids):
                ...
                subject = _decode_mime(em.get("Subject", ""))
                from_addr = _decode_mime(em.get("From", ""))
                body = _get_email_body(em)
                combined = subject + " " + body

                # 只处理 AWS 相关邮件
                is_aws = (
                    "awsapps.com" in from_addr.lower()
                    or "signin.aws" in from_addr.lower()
                    or "amazon" in from_addr.lower()
                    or "verify your identity" in subject.lower()
                    or "verification code" in combined.lower()
                )
                if not is_aws:
                    continue

                code = _extract_otp(combined)
                if code:
                    mail.logout()
                    print(f"{prefix}✅ 验证码: {code}")
                    return code
...
def main():
    print("=" * 60)
    print("Kiro / AWS Builder ID 自动注册工具")
    print("=" * 60)
    proxy = os.getenv("PROXY", "").strip()
    if not proxy or proxy.startswith("#"):
        proxy = None
    pwd = os.getenv("PASSWORD") or None
    name = os.getenv("DEFAULT_NAME") or "Kiro User"

    # 创建 HME 随机邮箱
    print("[HME] 创建新的 Hide My Email 地址...")
    try:
        email_addr = generate_hme()
    except RuntimeError as e:
        err_msg = str(e)
        if "retryAfter" in err_msg:
            print(f"\n❌ iCloud HME 限速，请稍后再试。\n   错误详情: {err_msg}")
        else:
            print(f"\n❌ HME 创建失败: {err_msg}")
        return
    use_password = pwd or _pwd()
    print(f"✅ 注册邮箱: {email_addr}")
    print(f"✅ 密码: {use_password}")
    print(f"✅ 验证码转发到: {IMAP_EMAIL}")
    print()

    try:
        reg = KiroRegister(proxy=proxy, tag="KIRO")
        reg.log(f"========== 开始注册: {email_addr} ==========")

        redir = reg.step1_kiro_init()
        if not redir:
            raise RuntimeError("InitiateLogin 失败")
        if not reg.step2_get_wsh(redir):
            raise RuntimeError("获取 wsh 失败")
        if not reg.step3_signin_flow(email_addr):
            raise RuntimeError("signin flow 失败")
        if not reg.step4_signup_flow(email_addr):
            raise RuntimeError("signup flow 失败")
        if not reg.profile_wf_id:
            raise RuntimeError("未获取到 workflowID")
        tes = reg.step5_get_tes_token()
        if not tes:
            reg.log("  ⚠️ TES token 获取失败, 继续...")
        if not reg.step6_profile_load():
            raise RuntimeError("profile start 失败")
        if reg.step7_send_otp(email_addr) is None:
            raise RuntimeError("send OTP 失败")

        otp = wait_for_otp(email_address=email_addr, timeout=240, tag=reg.tag)
        if not otp:
            raise RuntimeError("未获取到验证码")

        identity = reg.step8_create_identity(otp, email_addr, name)
        if not identity:
            raise RuntimeError("create-identity 失败")
        reg_code = identity["registrationCode"]
        sign_in_state = identity["signInState"]

        signup_registration = reg.step9_signup_registration(reg_code, sign_in_state)
        if not signup_registration:
            raise RuntimeError("signup registration 失败")
        password_state = reg.step10_set_password(use_password, email_addr, signup_registration)
        if not password_state:
            raise RuntimeError("设置密码失败")

        login_result = reg.step11_final_login(email_addr, password_state)
        if not login_result:
            reg.log("  ⚠️ 最终登录步骤失败, 但账号可能已创建成功")

        tokens = reg.step12_get_tokens()
        if not tokens:
            ...
            if device_tokens:
                ...
                reg.log("🎉 注册完成! (含 accessToken + sessionToken, 但 refreshToken 获取失败)")
                result = {
                    "email": email_addr, "password": use_password, "name": name,
                    "accessToken": tokens["accessToken"],
                    "sessionToken": tokens["sessionToken"],
        ...
        if result.get('accessToken'):
            print(f"accessToken: {result['accessToken'][:60]}...")
        if result.get('sessionToken'):
            print(f"sessionToken: {result['sessionToken'][:60]}...")
        if result.get('clientId'):
            print(f"clientId: {result['clientId'][:40]}...")
        if result.get('clientSecret'):
            print(f"clientSecret: {result['clientSecret'][:40]}...")
        if result.get('refreshToken'):
            print(f"refreshToken: {result['refreshToken'][:60]}...")

        if os.getenv("KIRO_JSON_OUTPUT"):
            print("###RESULT###" + json.dumps(result, ensure_ascii=False))
        else:
            with open("kiro_accounts.txt", "a", encoding="utf-8") as f:
